...
export const getOneProduct = async (req, res) => {
  try {
    ...
    const idQuery = mongoose.Types.ObjectId(toSearch)
    const findProduct = await KnProducts.findById(idQuery)
    if (!findProduct) return res.status(400).json({ msg: 'Item not found.' })
    res.json(findProduct)
  } catch (error) {
    res.status(500).json({ error: error.message })
  }
